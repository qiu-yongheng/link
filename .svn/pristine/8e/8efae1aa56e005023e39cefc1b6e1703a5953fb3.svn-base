 ...
 *
 */
class BoxSession(build: BaseBuilder<BoxSession>) : BaseSession(
    A3A4Link(build.packAdapter(BoxPackAdapter()).profileManager(BoxProfileManager())),
    build
) {
    ...
    override fun initConfig() {
        ...
        call(CommandManager.boxCommand.getKey(deviceKey))
            .timeout(1000)
            .enqueue(object : SessionCallback<KeyResult> {
                override fun onSuccess(
                    call: ISessionCall<KeyResult>,
                    data: IResp<KeyResult>
                ) {
    ...
    class Builder : BaseBuilder<BoxSession>() {
        override fun build(): BoxSession =
            BoxSession(this)
    }
}
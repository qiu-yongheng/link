...
public class BaseBoxLockServiceActivity extends Activity {
    private static final String TAG=BaseBoxLockServiceActivity.class.getSimpleName();

    protected BoxLockService mService=null;
    protected final ServiceConnection mServiceConnection= new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName name, IBinder service) {
            mService= ((BoxLockService.LocalBinder)service).getService();
     ...
     * @param mBLEService
     */
    protected  void onServiceConnectedCallBack(BoxLockService mBLEService){
    ...
    /**
     * 连接 柜锁
     * @param boxLockMac  柜锁MAC地址
     */
    protected  void connectBoxLock(String boxLockMac){
        mService.connect(boxLockMac);
    }
...
//        Log.i(TAG, "onItemClick: service activity  断开连接的 操作线程="+Thread.currentThread().getName());
     ...
     * 发送 柜锁 开锁指令
     */
    protected  void sendBoxLockOpenCommand(int uid,long timestamp){
        mService.sendBoxLockOpen(uid,timestamp);
    }
    ...
    private void registerLocalReceiver(){
        ...
        intentFilter.addAction(BoxLockService.ACTION_BLE_TRANSMISSION_PACK);
        ...
        intentFilter.addAction(BoxLockService.ACTION_BLE_DISCONNECTED);
    ...
    protected void startBindService(){
        ...
        Intent gattServiceIntent=new Intent(this,BoxLockService.class);
        //启动服务
...
//            Log.i(TAG, "onReceive: all  action="+intent.getAction());
            ...
            else if(BoxLockService.ACTION_BLE_BOX_LOCK_OPEN.equals(intent.getAction())){
                // 开匣广播
            ...
            }else if(BoxLockService.ACTION_BLE_BOX_LOCK_INFO.equals(intent.getAction())){
            ...
            }else if(BoxLockService.ACTION_BLE_BOX_LOCK_LOW_STATUS.equals(intent.getAction())){
                int status = intent.getIntExtra(BoxLockService.EXTRA_LOW_POWER_STATUS,0);

                onBLEBoxLockLowStatusCallBack( status);
            }
            else if(BoxLockService.ACTION_BLE_TRANSMISSION_PACK.equals(intent.getAction())){
//                Log.i(TAG, "onReceive: 数据传输");
                ...
                onBLEBoxLockTransmission(  pack,  type);
            }
        }
    };

    protected  void onBLEBoxLockTransmission(int pack,int type){

 ...
 *
 */
class CarportReadTask(private val session: ISession, listener: OnProgressListener<String>) : BaseTask<String>(listener) {
    ...
    override fun start() {
        AppExecutors.WORK_THREAD.execute {
            try {
                ...
                val packInfo = session.call(CommandManager.carportCommand.startRead()).execute()
    ...
    private fun transfer(deviceType: Int, crc: Int): ByteArray {
        ...
        while (finishPack < totalPack) {
            ...
            retry(5) { retry ->
                ...
                val result = session
                    .call(CommandManager.carportCommand.read(finishPack, deviceType))
                    .retry(0)
                    .timeout(1000)
                    .execute()
                    .getResult()

package com.omni.support.ble.task.bike
...
class BLWriteTask(
    ...
    private fun startWrite(totalPack: Int, crc: Int): Int {
        log("start write")
        val result =
            session.call(CommandManager.blCommand.startWrite(totalPack, crc, deviceType)).execute().getResult()
                ?: throw BleException(BleException.ERR_DATA_NULL, "启动下载失败")
            ...
            while (finishPack < totalPack) {
                ...
                retry(5) { retry ->
                    ...
                    if (finishPack == totalPack - 1) {
                        val result = session
                            .call(CommandManager.blCommand.endWrite(finishPack, data))
                            .retry(0)
                            .timeout(5000)
                            .execute()
                            .getResult()
                        nextPack = totalPack
                        result?.apply { finishCmd = cmd }
                        isSuccess = result != null
                    } else {
                        val result = session
                            .call(CommandManager.blCommand.write(finishPack, data))
                            .retry(0)
                            .timeout(5000)
                            .execute()
                            .getResult()
    ...
    private fun quitTransfer() {
        log("quit transfer mode: ${HexString.valueOf(finishCmd)}")
        if (finishCmd != 0) {
            session.call(CommandManager.blCommand.endTransmission(finishCmd)).execute().getResult()
        } else {
            throw BleException(BleException.ERR_QUIT_TRANSFER_FAIL, "退出写模式失败")
        }
    }
}
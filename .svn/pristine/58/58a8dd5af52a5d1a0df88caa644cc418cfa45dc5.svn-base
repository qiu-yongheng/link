...
class BLUpgradeTask(
    ...
    private fun startUpgrade(totalPack: Int, crc: Int): Int {
        ...
        val result =
            session.call(
                CommandManager.blCommand.startUpgrade(
                    totalPack,
                    crc,
                    deviceType,
                    updateKey
                ...
                ?: throw BleException(BleException.ERR_DATA_NULL, "启动升级失败")
    ...
    private fun upgrade(payload: Array<ByteArray>) {
        try {
            ...
            while (finishPack < totalPack) {
                ...
                retry(5) { retry ->
                    ...
                    if (finishPack == totalPack - 1) {
                        ...
                        val result = session
                            .call(CommandManager.blCommand.upgrade(finishPack, data))
                            .retry(0)
                            .timeout(5000)
                            .execute()
                            .getResult()
            ...
            log("end upgrade pack cmd = ${HexString.valueOf(finishCmd)}")
            // 退出升级模式
            quitTransfer()
        }
    }

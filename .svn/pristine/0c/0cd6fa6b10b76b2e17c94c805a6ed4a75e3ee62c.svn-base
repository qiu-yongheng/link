 ...
 *
 */
class CarportKeySession(build: BaseBuilder<CarportKeySession>) : BaseSession(
    A3A4Link(
        build.profileManager(CarportKeyProfileManager()).packAdapter(CarportPackAdapter())
    ), build
) {
    ...
    class Builder : BaseBuilder<CarportKeySession>() {
        override fun build(): CarportKeySession =
            CarportKeySession(this)
    }
}
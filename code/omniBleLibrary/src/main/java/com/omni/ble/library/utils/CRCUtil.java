...
public class CRCUtil {
	 ...
	 * @return
     */
	public static boolean CheckCRC2(byte[] data){
		byte[] t = new byte[data.length-2];
		System.arraycopy(data,2,t,0,data.length-2);
		int crcInt = calcCRC(t);
		Log.i(TAG,"CheckCRC2: crcInt" + crcInt);


		int dataCrcInt = ((data[0] &0xFF) <<8)  | (data[1]&0xFF);
		Log.i(TAG,"CheckCRC2: dataCrcInt=" + dataCrcInt);
		if(dataCrcInt == crcInt ) return true;
		return false;
	}
	...
	public static byte[] returnCRC(byte[] data)
	{
		...
		result[resultLen-1]=(byte) (ucCRCHi&0xFF);
		result[resultLen-2]=(byte) (ucCRCLo&0xff);
		return result;
	}
	public static byte[] updateCRC(byte[] sendData){
		int preval=0xffff;
		int ucCRCHi =(preval & 0xff00)>>8;
		int ucCRCLo = preval& 0x00FF;
		int iIndex;
		int resultLen=sendData.length;
		for(int i=0,len=resultLen-2;i<len;i++){
			iIndex = (ucCRCLo^sendData[i]) &0x00FF;
			ucCRCLo = ucCRCHi^t_crc16_h[iIndex];
			ucCRCHi = t_crc16_l[iIndex];
		}
		sendData[resultLen-1]=(byte) (ucCRCHi&0xFF);
		sendData[resultLen-2]=(byte) (ucCRCLo&0xff);
		return sendData;
